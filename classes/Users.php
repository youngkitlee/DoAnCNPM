...
class Users extends DBConnection
{
	...
	public function save_users()
	{
		...
		if (empty($id)) {
			...
			if ($qry) {
				...
				$this->settings->set_flashdata('success', 'Thông tin người dùng đã được lưu thành công.');
				foreach ($_POST as $k => $v) {
					if ($k != 'id') {
						if (!empty($data)) $data .= " , ";
			...
			if ($qry) {
				$this->settings->set_flashdata('success', 'Thông tin người dùng đã được cập nhật thành công.');
				foreach ($_POST as $k => $v) {
					if ($k != 'id') {
						if (!empty($data)) $data .= " , ";
						$this->settings->set_userdata($k, $v);
	...
	public function delete_users()
	{
		...
		if ($qry) {
			$this->settings->set_flashdata('success', 'Thông tin người dùng đã được xóa thành công.');
			return 1;
		} else {
			return false;
		}
	...
	public function save_fusers()
	{
		...
		if ($save) {
			$this->settings->set_flashdata('success', 'Thông tin người dùng được cập nhật thành công.');
			foreach ($_POST as $k => $v) {
				if (!in_array($k, array('id', 'password'))) {
					if (!empty($data)) $data .= " , ";
					$this->settings->set_userdata($k, $v);
